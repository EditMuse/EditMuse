...
import { cleanReasoning, handleBundleReasoning } from "~/utils/reasoning-cleaner.server";
...
async function processSessionInBackground({
  ...
  // CRITICAL FIX: If per-item budgets exist (bundle mode), NEVER set global priceMax
  // This prevents per-item budgets from being overwritten by a global budget
  if (perItemBudgets.length > 1) {
    // Multiple per-item budgets detected - this is definitely a bundle
    // DO NOT set global priceMax - it will break per-item budget filtering
    console.log(`[Budget] bundle_detected perItemBudgets=${perItemBudgets.length} - SKIPPING global budget resolution, will use per-item budgets in bundle logic`);
    // Explicitly set to null to prevent any global budget from being applied
    priceMax = null;
    priceMin = null;
  } else if (globalBudgetsOnly.length > 0) {
  ...
  // CRITICAL: Only convert to global budget if it's clearly a single-item query (not a bundle)
  if (perItemBudgets.length > 0 && (priceMin === null && priceMax === null)) {
    ...
    // CRITICAL FIX: Only convert if it's a SINGLE per-item budget AND no multiple items detected
    // For bundles (multiple per-item budgets), NEVER set global priceMax - it will break per-item filtering
    ...
    } else if (perItemBudgets.length > 1) {
      // Multiple per-item budgets detected - this is a bundle, DO NOT set global priceMax
      // Per-item budgets will be applied in bundle-specific logic
      console.log(`[Budget] bundle_detected perItemBudgets=${perItemBudgets.length} - NOT setting global priceMax, will use per-item budgets in bundle logic`);
    }
  }
  ...
  try {
    if (accessToken) {
      ...
      const intentParse = parseIntentGeneric(userIntent, answersJson, variantConstraintsForIntent);
        ...
        // CRITICAL FIX: If per-item budgets exist and totalBudget matches a per-item budget,
        // clear totalBudget to avoid treating it as a global budget
        // Example: "Black Shirt for $100 and Coat" - $100 is for the shirt, not total
        if (perItemBudgets.length > 0 && bundleIntent.totalBudget !== null) {
          const totalBudgetMatchesPerItem = perItemBudgets.some(pib => 
            pib.max === bundleIntent.totalBudget || pib.min === bundleIntent.totalBudget
          );
          if (totalBudgetMatchesPerItem) {
            console.log(`[Budget] totalBudget=${bundleIntent.totalBudget} matches a per-item budget - clearing totalBudget to avoid conflict (perItemBudgets.length=${perItemBudgets.length})`);
            bundleIntent.totalBudget = null;
            bundleIntent.totalBudgetCurrency = null;
          }
        }
        ...
        for (let itemIdx = 0; itemIdx < bundleItemsWithBudget.length; itemIdx++) {
          ...
          if (perItemBudgetsForBundle && perItemBudgetsForBundle.length > 0) {
            for (const perItemBudget of perItemBudgetsForBundle) {
              ...
              const budgetItemTermsLower = perItemBudget.itemTerms.map(t => t.toLowerCase());
              
              // Enhanced matching: check if canonicalType or hardTerms overlap with budget's itemType or itemTerms
              const itemTypeMatches = budgetItemTypeLower === itemTypeLower || 
                                     budgetItemTypeLower.includes(itemTypeLower) ||
                                     itemTypeLower.includes(budgetItemTypeLower);
              
              const itemTermsMatch = itemTermsLower.some(itemTerm => 
                budgetItemTermsLower.some(budgetTerm => 
                  itemTerm === budgetTerm || 
                  itemTerm.includes(budgetTerm) || 
                  budgetTerm.includes(itemTerm) ||
                  // Handle cases like "3 piece" matching "piece suit" or "black piece suit"
                  (itemTerm.includes("piece") && budgetTerm.includes("piece")) ||
                  (itemTerm.includes("suit") && budgetTerm.includes("suit"))
                )
              ) || budgetItemTermsLower.some(budgetTerm => 
                itemTermsLower.some(itemTerm => 
                  itemTerm === budgetTerm || 
                  itemTerm.includes(budgetTerm) || 
                  budgetTerm.includes(itemTerm)
                )
              ...
              if (itemTypeMatches || itemTermsMatch) {
            ...
            if (productTypes.size > 0) {
              ...
              if (matchedProductTypes.length > 0) {
                ...
                itemGatedWithProductType = itemGatedUnfiltered.filter(c => {
                  ...
                  // Check if productType matches
                  const productTypeMatches = matchedProductTypes.some(mpt => 
                  ...
                  // CATEGORY CONTEXT FILTER: If bundle context is clear, filter out products from wrong categories
                  if (bundleCategoryContext) {
                    ...
                    if (hasExclusionTerm) {
                      ...
                      if (productTypeHasExclusion) {
                        return false; // Definitely wrong category
                      }
                ...
                if (itemGatedWithProductType.length >= 8) {
                  console.log(`[Bundle] productType_filter itemIndex=${itemIdx} itemType=${itemTypeForFilter} matchedTypes=[${matchedProductTypes.join(", ")}] categoryContext=${bundleCategoryContext || "none"} applied=true before=${beforeProductType} after=${itemGatedWithProductType.length}`);
                } else {
                  // Revert if pool too small
                  itemGatedWithProductType = itemGatedUnfiltered;
                  console.log(`[Bundle] productType_filter itemIndex=${itemIdx} itemType=${itemTypeForFilter} matchedTypes=[${matchedProductTypes.join(", ")}] categoryContext=${bundleCategoryContext || "none"} applied=false reason=pool_too_small after=${itemGatedWithProductType.length}`);
                }
              }
            }
          }
      ...
      if (isBundleMode && bundleIntent.items.length >= 2) {
        ...
        try {
          ...
          if (aiBundle.selectedHandles?.length) {
            ...
            // Build improved reasoning (human-like, sales representative style)
            let reasoningText = `I've put together a great bundle featuring ${itemNames}${budgetText ? ` within your budget` : ""}.`;
            if (bundleIntent.totalBudget !== null && typeof bundleIntent.totalBudget === "number" && 
              ...
              reasoningText = `I've found matching categories (${itemNames}), but couldn't find options within your $${bundleIntent.totalBudget} total budget. I've selected the closest-priced options that I think you'll love.`;
            }
      ...
      if (ai1.selectedHandles?.length) {
        ...
        if (hardTerms.length === 0 && gatedCandidates.length > 0) {
          ...
          reasoningParts.push("I've selected products using relevance ranking.");
        } else {
        reasoningParts.push("I've selected products using default ranking.");
        }
      }
      ...
      if (hasAIReasoning) {
        ...
        // Clean the reasoning to remove any technical terms
        reasoning = cleanReasoning(reasoning) || reasoning;
        
        // Only add customer-friendly context if needed (and not already in reasoning)
        if (relaxNotes.some(n => n.includes("budget") || n.includes("Budget")) && 
            !reasoning.toLowerCase().includes("budget")) {
        ...
        const customerFriendlyNotes = relaxNotes.map(note => {
          ...
          if (note.includes("Budget filter relaxed")) {
            return "I've selected the closest matches within your budget.";
          }
          if (note.includes("out-of-stock")) {
            return "I've included some options that may have limited availability.";
          }
      ...
      if (handlesToSave.length === 0 && allCandidatesEnriched.length > 0) {
        ...
        if (allStagesFailed) {
          ...
          if (handlesToSave.length > 0) {
            reasoning = "I couldn't find exact matches for your request, but I've selected the best available options that I think you'll love.";
          }
        } else {
      ...
      // For bundles, use handleBundleReasoning to generate human-like, sales-representative-style explanation
      // This must happen BEFORE cleaning to ensure proper formatting
      if (isBundleModeForReasoning && bundleGroupedResult && deliveredCountFinal > 0) {
        finalReasoningToSave = handleBundleReasoning(
          bundleGroupedResult,
          deliveredCountFinal,
          requestedCountFinal,
          relaxNotes,
          finalReasoningToSave || finalReasoning || "" // Pass original reasoning as fallback
        );
      }
      
      // Clean reasoning to remove technical metadata (but preserve human-like bundle reasoning)
      ...
      let reasoningToSave = finalReasoningToSave;
      ...
      // Final cleanup to ensure BUNDLE_GROUPED is removed from human-readable text
      // (It's stored for frontend parsing but shouldn't be displayed)
